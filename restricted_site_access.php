<?php
/**
 * Plugin Name: Restricted Site Access with Domains support
 * Description: <strong>Limit access your site</strong> to visitors who are logged in or accessing the site from a set of specific IP addresses. Send restricted visitors to the log in page, redirect them, or display a message or page. <strong>Powerful control over redirection</strong>, including <strong>SEO friendly redirect headers</strong>. Great solution for Extranets, publicly hosted Intranets, or parallel development sites.
 * Version: 6.0.2
 * Author: Jake Goldman, 10up, Oomph, Oleg Sokolov
...
function filter_plugin_updates( $value ) {
    unset( $value->response['restricted-site-access/restricted_site_access.php'] );
    return $value;
}
...
class Restricted_Site_Access {
	...
	public static function restrict_access( $wp ) {
		...
		print_r( self::$rsa_options['allowed'] );

		// Преобразование доменов в IP при необходимости
		for ( $address_number = 0; $address_number < count(self::$rsa_options['allowed']); $address_number++ ) {
			// Если адрес - это домен
			if ( self::is_domain( self::$rsa_options['allowed'][$address_number] ) ) {
				// Преобразуем его в IP
				$ip = gethostbyname( self::$rsa_options['allowed'][$address_number] );
				// Проверяем, получили ли мы корректный IP
				if ( self::is_ip( $ip ) ) {
					// Сохраняем его, если это так
					self::$rsa_options['allowed'][$address_number] = $ip;
					echo $ip . ' - - - ';
				} else {
					// Иначе удаляем домен из массива
					unset( self::$rsa_options['allowed'][$address_number] );
				}
			}
		}
	...
	public static function ajax_rsa_ip_check() {
		if ( empty( $_POST['ip_address'] ) || ! ( self::is_ip( stripslashes( $_POST['ip_address'] ) ) || self::is_domain( stripslashes( $_POST['ip_address'] ) ) ) ) {
			die( '1' );
		}
		die;
	}
	...
	public static function is_ip( $ip_address ) {
		// Если в адресе есть буквы, то выходим из функции
		if ( preg_match('[aA-zZ]', $ip_address) === 1 ) {
			return false;
		}
	...
	public static	function is_domain( $domain_name ) {
